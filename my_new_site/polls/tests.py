...
class QuestionIndexResultsTests(TestCase):
    ...
    def test_results_view_with_a_past_question_with_choices(self):
        """
        The results view of a question with a pub_date in the past should
        display the question's text and list of choices
        """
        past_question = create_question(question_text='Past Question.', days=-5)
        past_question.choice_set.create(choice_text='choice 1.', votes = 0)
        past_question.choice_set.create(choice_text='choice 2.', votes = 1)
        url = reverse('polls:results', args=(past_question.id,))
        response = self.client.get(url)
        self.assertContains(response, past_question.question_text)
        choice_list = list(past_question.choice_set.all())
        self.assertContains(response, choice_list[0].choice_text)
        self.assertContains(response, choice_list[1].choice_text)
